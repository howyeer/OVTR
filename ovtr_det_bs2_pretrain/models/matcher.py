...
class ctr_HungarianMatcher(HungarianMatcher):
    ...
    def hungarian(self, bs, pred_logits, pred_boxes, tgt_ids_all, tgt_bbox, sizes):
        # We flatten to compute the cost matrices in a batch
        out_prob = pred_logits[:, :, tgt_ids_all].flatten(0, 1).sigmoid()
    ...
    def forward(self, outputs, targets, select_id):
        # We flatten to compute the cost matrices in a batch
        bs, self.num_queries = outputs["pred_logits"].shape[:2]
        ...
        if "pred_boxes_for_matching_pre" in outputs:
            pred_boxes = outputs["pred_boxes_for_matching_pre"]
        else:
            pred_boxes = outputs["pred_boxes"]

        indices = self.hungarian(bs, pred_logits, pred_boxes, tgt_ids_all, tgt_bbox, sizes)
        return [(torch.as_tensor(i, dtype=torch.int64), torch.as_tensor(j, dtype=torch.int64)) for i, j in indices]
