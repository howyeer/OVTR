...
class Transformer(nn.Module):
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None, text_dict=None, cache=None):
        ...
        if self.two_stage_type == "standard":
            ...
            print("why not two_stage")

        ###### Begin Decoder ######
        hs_cti, hs_ofa, inter_references, pre_outputs_classes, encoder_outputs_class = self.decoder(
            tgt.transpose(0, 1), 
            ...
            enc_output_undetach=tgt_undetach.unsqueeze(0)
            )
        
        if self.two_stage_type == "standard":
            ref_enc = topk_coords_unact_undetach.sigmoid().unsqueeze(0)
        else:
            ref_enc = None
                                              
        inter_references_out = inter_references
        if self.two_stage_type == "standard":
            return (hs_cti, hs_ofa, init_reference_out, inter_references_out, ref_enc, pre_outputs_classes, encoder_outputs_class)

        return hs_cti, init_reference_out, ref_enc, pre_outputs_classes
...
class TransformerDecoder(nn.Module):
    ...
    def pre_class_embed(self, output, text_dict, layer_id=-1, encoder=False):
        ...
        outputs_class = self.logits_with_bias(outputs_class, self.text_bias[layer_id,:,:self.num_queries,:], self.log_scale_cls[layer_id,:,:self.num_queries,:]) 
    ...
    def forward(self, tgt, reference_points, src, src_spatial_shapes, src_level_start_index, src_valid_ratios,
        ...
        self.num_queries = tgt.shape[0]
        self.select_text_num = text_dict["select_text_num"]
        self.text_bias, self.log_scale_cls = self.get_logits_bias(text_dict["encoded_text"], self.num_queries)

        enc_outputs_class = self.pre_class_embed(enc_output_undetach, text_dict, encoder = True)
